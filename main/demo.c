...
//设置休眠时间
uint64_t sleep_time_ok=60;//正常休眠时间(秒)
uint64_t sleep_time_err=30;//错误休眠时间(秒)
...
void sleep_time_gpio_wakeup(uint64_t sleep_time, gpio_num_t gpio_num, struct_gpio_wakeup_type gpio_wakeup_type) 
{
    ...
    {
        //使能定时唤醒 us(可选设置)
        gy_sleep_timer(sleep_time);//设置定时唤醒
    }
...
camera_config_t camera_config_init = {
    ...
    .frame_size = FRAMESIZE_QXGA,    //设置照片分辨率
    .jpeg_quality = 2, //压缩率(取值范围0-63);  值越小,图片质量越好
...
static void http_camera_task(void *pvParameters)
{
    ...
    while (true)
    {
            ...
            /*配置图像亮度参数*/
            sensor->set_brightness(sensor, 1);       // 亮度(-2到2), 0为默认
            // sensor->set_contrast(sensor, 0);         // 对比度(-2到2), 0为默认
            // sensor->set_saturation(sensor, 0);       // 饱和度(-2到2), 0为默认
            sensor->set_gainceiling(sensor, 30);     // 增益上限(0-30)
            sensor->set_gain_ctrl(sensor, 1);         // 启用自动增益
            sensor->set_exposure_ctrl(sensor, 0);     // 启用自动曝光
            sensor->set_aec_value(sensor, 200);       // 手动曝光值(0-1200), 越小越亮
            sensor->set_wb_mode(sensor, 1);           // 白平衡模式(0=自动)
            sensor->set_ae_level(sensor, 2);          // 曝光等级(-2到2), 0为默认
...
static void init_sntp(void)
{
    ...
    // 设置系统初始时间为2020年1月1日（避免系统时间过大导致SNTP同步异常）
    struct timeval tv = {
    ...
    sntp_set_sync_mode(SNTP_SYNC_MODE_SMOOTH);//设置SNTP时间同步模式：平滑模式。
    sntp_init();
}
static void sntp_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if((timeinfo.tm_year+1900)>2025 && timeinfo.tm_year+1900<2030)//获取到网络时间
        {
            sntp_stop();
            while(1)
            {
