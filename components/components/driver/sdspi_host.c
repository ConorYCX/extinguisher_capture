...
{
    if (slot->gpio_cs!=0xff) gpio_set_level(slot->gpio_cs, 1);
}

/// Set CS low for given slot
static void cs_low(slot_info_t *slot)
{
    if (slot->gpio_cs!=0xff) gpio_set_level(slot->gpio_cs, 0);
}
...
esp_err_t sdspi_host_init_device(const sdspi_device_config_t* slot_config, sdspi_dev_handle_t* out_handle)
{
    ...
    *slot = (slot_info_t) {
    ...
    if (slot_config->gpio_cs!=0xFF)
    {
        io_conf.mode = GPIO_MODE_OUTPUT;
        io_conf.pin_bit_mask = 1ULL << slot_config->gpio_cs;
